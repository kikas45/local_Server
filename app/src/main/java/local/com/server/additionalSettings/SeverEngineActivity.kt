...
import local.com.server.databinding.ActivityDeMo202100Binding
import local.com.server.databinding.CustomPortLayoutBinding


class SeverEngineActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.textUseZipFolder.setOnClickListener {
            Utility.hideKeyBoard(applicationContext, binding.editTextPath)
            startActivity(Intent(applicationContext, ExtractZipActivity::class.java))
            finish()
        ...
        binding.imageWiFiOn.setOnClickListener {
            Utility.hideKeyBoard(applicationContext, binding.editTextPath)
            val intent = Intent(Settings.ACTION_WIFI_SETTINGS)
            startActivity(intent)
        }

        binding.imageWiFiOFF.setOnClickListener {
            Utility.hideKeyBoard(applicationContext, binding.editTextPath)
            val intent = Intent(Settings.ACTION_WIFI_SETTINGS)
            startActivity(intent)
        }
    ...
    private fun showPopSetPort() {
        ...
        if (alertDialog.window != null) {
            ...
            alertDialog.window!!.attributes.windowAnimations =
                local.com.server.R.style.PauseDialogAnimation
        }
            ...
            if (Utility.isNetworkAvailable(applicationContext)) {
                val dialog = AlertDialog.Builder(this)
                    ...
                    .setPositiveButton("Stop & Restart") { d, _ ->
                        ...
                        callServiceStop()
                        handler.postDelayed({ callServiceStart() }, 5000)
                    }
...
*/
                ...
                val dialog = AlertDialog.Builder(this)
                    ...
                    .setPositiveButton("Stop & Restart") { d, _ ->
                        ...
                        lifecycleScope.launch {
                            try {
                                callServiceStop()
                                delay(2000) // ⏳ safer than handler.postDelayed
                                callServiceStart()
                            } catch (e: Exception) {
                                Log.d("ServerService", "Restart failed: ${e.message}")
                                Toast.makeText(
                                    this@SeverEngineActivity,
                                    "Failed to restart server. Try manually.",
                                    Toast.LENGTH_LONG
                                ).show()
                            }
                        }
        ...
        override fun onReceive(context: Context, intent: Intent) {
            try {
                ...
                if (activeNetworkInfo != null && activeNetworkInfo.isConnected) {
                    ...
                    binding.imageWiFiOn.visibility = View.VISIBLE
                    binding.imageWiFiOFF.visibility = View.INVISIBLE
                    setUpServerNetWorkDetector()

                } else {
                    // No internet Connection
                    binding.imageWiFiOn.visibility = View.INVISIBLE
                    binding.imageWiFiOFF.visibility = View.VISIBLE
                    setUpServerNetWorkDetector()
                }
                // No internet Connection
    ...
    private fun navigateBack() {
        ...
        val getNavigation = sharedBiometric.getString(Constants.KEY_NAVIGATOR, "").toString()
        if (getNavigation == Constants.SettingMaintenaceActivity){
            val intent = Intent(applicationContext, SettingMaintenaceActivity::class.java)
            startActivity(intent)
            finish()
