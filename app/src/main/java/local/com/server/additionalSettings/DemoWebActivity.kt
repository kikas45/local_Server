...
import local.com.server.databinding.ActivityDemoWebBinding
import local.com.server.databinding.CustomExitOrNotBinding
import local.com.server.databinding.CustomOfflinePopLayoutBinding
...
class DemoWebActivity : AppCompatActivity() {
            ...
            if (isValidLocalAddress(localUrl)) {
                if (Utility.isNetworkAvailable(applicationContext)) {
                    ...
                    show_Pop_Confirm_Exit(
                        "Attention!",
                        "No Network detected. The server requires a connection to your local network (Wi-Fi or Ethernet) to function properly. Please connect and try again."
                    )
                }
    ...
    private fun show_Pop_Confirm_Exit(title: String, body: String) {
        val bindingCD: CustomExitOrNotBinding = CustomExitOrNotBinding.inflate(layoutInflater)
        val alertDialogBuilder = android.app.AlertDialog.Builder(this)
        alertDialogBuilder.setView(bindingCD.root)
        ...
        bindingCD.textSucessful.text = title
        bindingCD.textBodyMessage.text = body

        if (body == Constants.SEVER_POP_MESSAGE) {
            bindingCD.textLaunchMyOffline.text = "Fix Now"
        }

        bindingCD.imgCloseDialogForegetPassword.setOnClickListener {
            alertDialog.dismiss()
            loadOfflinePageFromAssets("page_unreachable.html")
        }

        bindingCD.textLaunchMyOffline.setOnClickListener {
            sharedPrefs.edit().putString(Constants.KEY_NAVIGATOR, Constants.DemoWebActivity).apply()
            alertDialog.dismiss()
            startActivity(Intent(applicationContext, SeverEngineActivity::class.java))
            finish()
        }

        // ✅ Handle back button or outside tap dismiss
        alertDialog.setOnCancelListener {
            loadOfflinePageFromAssets("page_unreachable.html")
        }

        alertDialog.setOnDismissListener {
            // Extra safety — also covers dismiss via back button
    ...
    private fun loadOfflinePageFromAssets(fileName: String) {
        binding.SimpleProgressBar.visibility = View.GONE
        val offlinePage = "file:///android_asset/$fileName"
        binding.myWebview.loadUrl(offlinePage)
    }
    ...
    override fun onResume() {
        ...
        val hideBottomBar = sharedPrefs.getBoolean(Constants.KEY_HIDE_BAR, false)
        binding.bottomServerLayout.visibility = if (hideBottomBar) {
            View.VISIBLE
        } else {
            View.GONE
        }
    ...
    private fun navigateBackToSetting() {
        ...
        lifecycleScope.launch {
            ...
            val intent = Intent(this@DemoWebActivity, SettingMaintenaceActivity::class.java)
            startActivity(intent)
            finish()
        }
