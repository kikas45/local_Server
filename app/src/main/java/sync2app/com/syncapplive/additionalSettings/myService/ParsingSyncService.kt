...
class ParsingSyncService : Service() {
    ...
    private fun getZipDownloadsManually(sn: String, folderName: String, fileName: String) {
        ...
        val getSavedEditTextInputSynUrlZip = sharedP.getString(Constants.getSavedEditTextInputSynUrlZip, "").orEmpty()

        var replacedUrl = getSavedEditTextInputSynUrlZip
        if (getSavedEditTextInputSynUrlZip.contains("/App/index.html")) {
            replacedUrl = getSavedEditTextInputSynUrlZip.replace("/App/index.html", "/$folderName/$fileName")
        } else {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                if (result) {
                    withContext(Dispatchers.Main) {
                        ...
                        val request = DownloadManager.Request(Uri.parse(replacedUrl)).apply {
                            ...
                            setDestinationUri(Uri.fromFile(targetFile)) // ✅ Scoped Storage destination
                        }
                ...
                withContext(Dispatchers.Main) {
                    getNextOnFailedDownload(sn = sn, folderName = folderName, fileName = fileName)
                    Log.d(KoloLog, "Download failed: ${e.message}")
                }
            }
        }
    }
    ...
    private fun getParsingFilesDownload(sn: String, folderName: String, fileName: String) {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                if (result) {
                    withContext(Dispatchers.Main) {
                        ...
                        val request = DownloadManager.Request(Uri.parse(getFileUrl)).apply {
                            setTitle(fileName)
                            setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
                            setDestinationUri(Uri.fromFile(targetFile)) // ✅ Scoped-safe destination
                        }
                ...
                withContext(Dispatchers.Main) {
                    getNextOnFailedDownload(sn = sn, folderName = folderName, fileName = fileName)
                    Log.d(KoloLog, "Download error: ${e.message}")
                }
            }
        }
    }
